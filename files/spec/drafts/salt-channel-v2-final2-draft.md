...
* To Daniel Bernstein et al for developing the TweetNaCl cryptography library.
* To Kenneth Pernyer and Felix Grape for comments and implementations
  in Swift and JavaScript.
* To Felix Grape for carefully reviewing the protocol and its specification.
* To Håkan Olsson for comments and discussions.
* To Assured AB for multiple security reviews.
...
WebSocket, RS485, NFC, and Bluetooth Low Energy (BLE).

This document specifies the second version of the protocol -- *Salt Channel v2*.

Salt Channel is *Powered by Curve25519*. The cryptographic algorithms used
are those provided by TweetNaCl: ed25519 with for signatures,
x25519+xsalsa20+poly1305 for authenticated encryption, and SHA-512
for secure hashing.
...
with v1 and replaces it. New applications must not use v1 and existing
v1 applications should be migrated to v2.

The major changes are:

1. Removal of the Binson dependency.
2. Signature1, Signature2 modified to include hash(M1) and hash(M2).
3. The *protocol info* feature (A1A2 session) introduced.

The Binson dependency is removed to make the protocol independent
of that specification. Also, it means more fixed message sizes and offsets
...
messages and it is possible to ensure confidentiality of all messages
except for M1 and M2.

The protocol info feature (A1A2 session) allows a client to ask a server to
tell what protocols and protocol versions it supports before an actual
communication session is started by the client. This allows easy future
Salt Channel version upgrades since both the client and the server MAY
support multiple versions in parallel. Note that the A1A2 session
actually form a protocol that can be used independently of Salt Channel.
...
This section describes the design goals of the protocol and its scope.
...
* **Internet-capable**.
    ...
    redirect, delay all messages sent between any pair of peers. The attacker has
    access to every Salt Channel message ever sent and all messages from
...
* **Delay attack protection**.
    ...
    delays a package, the receiving peer should be able to detect this if the
    delay is abnormal.
...
* **No certificates**.
    ...
    Using certificates together with the public keys
    is certainly possible, but not included in the scope.

* **No quantum security**.
    The protocol is not intended to be secure for an
    attacker with a large quantum computer.
...
*of known size*. The underlying layer must provide an order-preserving
...
binary data either as an *ArrayBuffer* (byte array-like object) or a *Blob*
...
is strongly RECOMMENDED when Salt Channel is implemented over any other
similar type of byte stream; for example for
Salt Channel over RS232 ("serial port"), Salt Channel over RS485,
and Salt Channel over a Unix pipe.
...
the ordinary session for exchanging application messages and
the A1A2 session for querying a server for its available protocols.
...
and the client finishes the handshake by sending M4.
For applications where the client sends the first application message
(the most common case), this message SHOULD be sent together with M4
to achieve a single round-trip overhead (instead of two). Application
layer messages (AppPacket:s) are sent by both the client and the
...
session allows the client to ask the server about its public protocol
information. The session consists of message A1 from the client followed
by the A2 response from the server:

    Session = A1 A2

The session is finished after message A2. Note that there is no confidentiality
...
The protocol is designed so that both (compliant) peers will be able to
agree on when a Salt Channel ends.

The underlying reliable channel MAY or MAY NOT be reused for multiple sequential
Salt Channel sessions. If such reuse is allowed is not out of scope for
this specification.
...
A Salt Channel session also ends if a peer receives data that does
not follow the protocol. This is called a BAD PEER condition and is
...
If a peer receives data from the other peer that does not follow
this protocol, a *BAD PEER condition* arises. This means that the peer
determines that the other peer does not follow this protocol.
Examples of when this condition occurs includes:

* An EncryptedMessage is received and its Body field that cannot
  be successfully decrypted.

* An message is received with a PacketType that does not match any
  expected message type.

* A signature in message M3 or M4 is not valid.
...
* A message is received that is larger than the implementation can handle.
  ...
  case in the BAD PEER condition for convenience.

Whenever a peer determines that the other peer does not follow
protocol, we have a BAD PEER condition. When a BAD PEER condition is
detected by a peer, the peer MUST NOT send any further data to the
other peer (the bad peer) and MUST consider the session as ended.
So the error handling is simple for BAD PEER conditions, the session
is simply considered ended. Depending on the particular underlying
layer, the underlying communication may be closed.

The peer MAY log BAD PEER conditions that arises. If they are logged,
implementations should consider the ramifications with regards to
...
This section describes how different packets are
...
The term *message* is used for a whole byte array message sent between
the peers and *packet* is used to refer to any byte array --
...
in the sense that it MAY independently exist or not. Whether a field's
existence is optional, mandatory or forbidden could depend on other
fields and/or the state of the communication session so far.
...
* **WS**, RFC 6455, *The WebSocket Protocol*. December 2011.
